        ...
        mysql_connection.query(url,(error, topicRows) => {
            if(!error) {
                ...
                if(topicRows.length === 1) {
                    let subUrl = "SELECT accounts.user_name,discussion_reply.reply_id,discussion_reply.reply_content,discussion_reply.reply_created_time FROM discussion_reply LEFT JOIN accounts ON discussion_reply.user_id = accounts.user_id WHERE discussion_reply.topic_id="+topic_id+" ORDER BY reply_created_time DESC";
...
module.exports = {getAllDiscussionsData, getDiscussionData, createDiscussion, deleteDiscussion};